...
![Decanting the Web: the opening of the interactive data story](preview.png?v=2)
...
The site is **vanilla HTML + ES modules + D3 v7** (loaded from a CDN). There is **no build step**. Because it uses `fetch()` for the data, you must serve it over HTTP, because opening `index.html` from the file system will be blocked by the browser.
